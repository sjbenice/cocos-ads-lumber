...
export class CSG {
	...
	static toMesh(csg:CSG) : Mesh {
		if (csg && csg.polygons && csg.polygons.length > 0) {
            let iGeometry: IGeometry = { positions: [], indices: [], normals: [] };
            ...
            // }
            // if (hull.uvs) {
            //     iGeometry.uvs = writeAttribute(hull.uvs);
            // }
	...
	static cube(options: { center?: [number, number, number]; radius?: number | [number, number, number] } = {}): CSG {
		...
		return CSG.fromPolygons([
			...
			return new Vertex(pos, new Vector(info[1][0],info[1][1],info[1][2]));
		}))));
	}
	...
	static polyBox(height:number, xz:number[], buildTopBotton:boolean=true, center?:Vec3) : CSG {
		...
		const point = (angle:number, stack: number, x: number, z:number, normalBlend: number) => {
			const out = axisX.times(Math.cos(angle)).plus(axisY.times(Math.sin(angle)));
			const normal = out.times(1 - Math.abs(normalBlend)).plus(axisZ.times(normalBlend));
			return new Vertex(new Vector(x + c.x, (stack ? yTop : yBottom) + c.y, z + c.z), normal);
		};
		...
		for (let index = 0; index < vertices.length; index++) {
			...
			polygons.push(new Polygon([
				point(angle, 0, p1.x, p1.y, 0), 
				point(angle, 0, p0.x, p0.y, 0), 
				point(angle, 1, p0.x, p0.y, 0), 
				point(angle, 1, p1.x, p1.y, 0)]));// side
		}
		...
		if (buildTopBotton) {
			...
			if (triangles) {
				triangles.forEach(triangle => {
					polygons.push(new Polygon(triangle.map(v => point(0, 1, v.x, v.y, 1))));// top
					triangle.reverse();
					polygons.push(new Polygon(triangle.map(v => point(0, 0, v.x, v.y, -1))));// bottom
				});
			}
		}
